...
```java
// CEP 패턴 정의
// A 이벤트 다음 바로 B 이벤트가 10초 이내 발생됨
Pattern<Event, ?> pattern = Pattern.<Event>begin("A") // 패턴 시작
        // 패턴 정의
        .where(SimpleCondition.of(value -> value.getName().equals("error"))) // A 이벤트 정의 : 이름이 error인 경우 
        // 패턴 조합
        .next("B") // B 이벤트가 옴
        // 패턴 정의
        .where(SimpleCondition.of(value -> value.getName().equals("critical"))) // B 이벤트 정의 : 이름이 critical인 경우
        // 패턴 조합
